...
export default function TasksView({ tasks, onAddTask, onUpdateTaskStatus }) {
  const { deleteTask, projects, selectedProjectId, selectedProject, setActiveTab, showToast, activeRole } = useApp();
  const { currentUser } = useAuth();
  const effectiveRole = currentUser?.role || activeRole;
  const isExecutive = effectiveRole === 'management';
  const canCreateTask = ['admin', 'pm', 'management'].includes(effectiveRole);
  const canUpdateStatus = ['admin', 'pm', 'site_eng', 'management'].includes(effectiveRole);
  const canDeleteTask = ['admin', 'pm', 'management'].includes(effectiveRole);
